...
const AptitudeProblem = React.memo(() => {
  ...
  const getTopicColor = useCallback((topic) => {
    switch (topic.toLowerCase()) {
      case 'quantitative': return darkMode ? 'bg-purple-900 text-purple-200' : 'bg-purple-100 text-purple-800';
      case 'logical': return darkMode ? 'bg-blue-900 text-blue-200' : 'bg-blue-100 text-blue-800';
      case 'verbal': return darkMode ? 'bg-orange-900 text-orange-200' : 'bg-orange-100 text-orange-800';
      case 'analytical': return darkMode ? 'bg-teal-900 text-teal-200' : 'bg-teal-100 text-teal-800';
      default: return darkMode ? 'bg-gray-700' : 'bg-gray-200';
    }
  }, [darkMode]);

  const aptitudeProblem = useMemo(() => ({
    id: 1,
    ...
    explanation: "In a deck of 52 cards, there are 4 kings and 4 queens. So total favorable outcomes = 4 + 4 = 8. Total possible outcomes = 52. Probability = 8/52 = 2/13.",
    options: [
      { id: 'A', text: "1/13" },
      { id: 'B', text: "2/13" },
      { id: 'C', text: "1/4" },
  ...
  useEffect(() => {
    setSelectedProblem(aptitudeProblem);
    setOutput('');
  }, [aptitudeProblem]);
        ...
        >
          <div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              >
                {selectedProblem.topic} Aptitude
              </span>
              <span
                className={`px-2 py-1 rounded text-xs ${
...
export default AptitudeProblem;